<div>
    <div>
        <flux:text class="text-xl font-bold text-secondary">Proveedores</flux:text>
        <flux:breadcrumbs>
                            ...
                            <td class="px-4 py-3 text-center">
                                <flux:tooltip content="Editar">
                                    <flux:button variant="primary" size="xs" icon="pencil" class="cursor-pointer"
                                        wire:click="editModal({{ $provider->id }})"
                                        wire:target="editModal({{ $provider->id }})" />
                                </flux:tooltip>

                                <flux:tooltip content="Eliminar">
                                    <flux:button size="xs" icon="trash"
                                        class="cursor-pointer !bg-black !text-white"
                                        wire:click="deleteModal({{ $provider->id }})"
                                        wire:target="deleteModal({{ $provider->id }})" />
                                </flux:tooltip>
                            </td>
                        </tr>
                    @empty
    ...
    <x-modal name="form-modal" max-width="xl">
        <livewire:admin.provider.modals.form-modal />
    </x-modal>

    <x-modal name="delete-modal" max-width="md">
        <livewire:admin.provider.modals.delete-modal />
    </x-modal>
</div>
